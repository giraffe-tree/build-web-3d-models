...
- List 5–12 identity-critical features and map each to geometry, material/texture, hierarchy, or an explicit omission. Record each as a triple: the visible cue, its attachment/construction logic, and the proof view that will isolate it.
- Declare the mechanical type of every part connection (bolted, welded, press-fit, hinged, bearing, glued, or monolithic) and bind each to a dedicated close proof view. A joint implied only by tangency plus a color change reads as unattached and counts as not established in review.
- For repeated or hierarchical structures (branches, leaf clusters, modules, fasteners), author one master sample to final quality and review it before instancing or propagating; a defect in the master multiplies across the whole assembly.
...
- for every declared motion or interaction state, a fixed-camera sequence of at least five consecutive frames at fixed times (start through endpoint) showing the mechanism actually moving, registered in the manifest's `motionEvidence` field; a single still or a static pose screenshot is not interaction evidence;
